...
export default defineEventHandler(async (event) => {
  ...
  try {
    const text = await $fetch<string>(
      `${apiHost}/api/v1/public/tenants/${encodeURIComponent(tenant)}/llms-full.txt?language=${encodeURIComponent(lang)}`,
      { responseType: 'text' },
    )
    ...
    throw createError({ statusCode: 502, message: '租户 llms-full.txt 暂不可用' })
  }
})
