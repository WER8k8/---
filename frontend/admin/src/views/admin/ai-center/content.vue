 ...
 */
<template>
  <YdPage title="AI 内容助手" subtitle="内容生成与 SEO 优化 · 智能创作" surface="elevated">
    <template #actions>
...
async function loadPrompts() {
  try {
    const raw = await apiGet<unknown>('/content');
    const rows = Array.isArray(raw)
      ? raw
      : (raw as { items?: Prompt[] })?.items ?? (raw as { data?: Prompt[] })?.data ?? [];
    if (Array.isArray(rows) && rows.length) {
      prompts.value = rows.map((row, i) => ({
        id: Number((row as Prompt).id ?? i + 1),
        name: String((row as Prompt).name ?? (row as { title?: string }).title ?? `条目 ${i + 1}`),
        category: String((row as Prompt).category ?? '创意写作'),
        content: String((row as Prompt).content ?? ''),
        usageCount: Number((row as Prompt).usageCount ?? 0),
        createTime: String((row as Prompt).createTime ?? (row as { created_at?: string }).created_at ?? ''),
      }));
    }
  } catch {
    /* 保留本地种子数据 */
