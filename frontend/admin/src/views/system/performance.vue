...
async function loadSecurityReport() {
  try {
    ...
    if (import.meta.env.DEV) console.error('Failed to load security report:', error);
  }
}
...
async function runSecurityAudit() {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.error('Security audit failed:', error);
    message.error('安全审计失败');
  } finally {
