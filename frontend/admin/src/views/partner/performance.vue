<template>
  <YdPage surface="elevated">
    <div class="agent-dashboard partner-theme coachpro-tertiary coachpro-tertiary--partner">
      <section class="topbar panel uj-glass-panel">
        <div>
          <p class="kicker">{{ greeting }}，{{ auth.username || '省级代理' }}</p>
          <h1>省代经营看板</h1>
          <p class="desc">省区渠道拓展、回款与下级代理协同总览</p>
        </div>
        <div class="topbar-actions">
          <a-tag color="gold">省代视图</a-tag>
          <a-button size="small" @click="router.push('/partner/account-opening')">客户开户</a-button>
          <a-button size="small" type="primary" @click="router.push('/partner/daily-report')">经营日报</a-button>
        </div>
      </section>
      ...
      <section class="table-grid">
        ...
        <article class="panel uj-glass-panel side-card">
          ...
          <a-button block @click="router.push('/partner/churn-warning')">查看流失预警</a-button>
        </article>
      </section>
    </div>
  </YdPage>
...
const roleAlerts = ref<{ level: string; title: string; desc: string }[]>([
  { level: 'ok', title: '加载代理业绩中…', desc: '提醒来自真实 API，非写死示例' },
]);
...
function calcBarHeight(value: number, max: number): number { return 8 + Math.round((value / max) * 72); }
...
async function loadClients() {
  const data = await apiGet<{ items?: ClientRow[] }>('/agent/clients', { search: clientSearch.value || undefined, status: clientStatusFilter.value, page: 1, page_size: 60 });
  clients.value = (data?.items ?? []).map((c) => ({ ...c, openDate: c.open_date, firstPayment: c.first_payment }));
}
const filteredClients = computed(() => clients.value);
function handleClientSearch() { loadClients(); }
function handleClientFilter() { loadClients(); }
...
onMounted(async () => {
  try { await Promise.all([loadDashboard(), loadTrends(), loadClients()]); } catch { /* keep static */ }
});
</script>

<style scoped lang="scss">
.agent-dashboard { padding: 4px 2px; }
.main-grid { display: grid; grid-template-columns: 1.2fr 1fr; gap: 12px; }
.trend-card,.summary-card,.table-card,.side-card { padding: 12px 14px; }
.card-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
.card-head h3 { margin: 0; font-size: 14px; font-weight: 700; color: #1f2937; }
.card-head span { font-size: 12px; color: #9ca3af; }
.line-trend { height: 140px; position: relative; border: 1px dashed #cfe0e8; border-radius: 10px; background: linear-gradient(180deg, rgba(126, 162, 181, 0.15), #fff); }
.trend-empty { height: 140px; display: flex; align-items: center; justify-content: center; color: #9ca3af; font-size: 13px; border: 1px dashed #e5e7eb; border-radius: 10px; }
.line-dot { position: absolute; width: 8px; height: 8px; border-radius: 50%; transform: translate(-50%, 50%); }
.line-dot--blue { background: #5f88a0; } .line-dot--amber { background: #6c98ad; }
.axis-row { margin-top: 6px; display: grid; grid-template-columns: repeat(6, 1fr); font-size: 11px; color: #9ca3af; text-align: center; }
.summary-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 8px; }
.summary-list li { display: flex; justify-content: space-between; align-items: center; border: 1px solid #eef2f7; border-radius: 10px; padding: 8px 10px; }
.summary-list p { margin: 0; font-size: 13px; color: #1f2937; font-weight: 600; }
.summary-list small { color: #6b7280; font-size: 11px; }
.summary-list strong { color: #0f172a; font-size: 13px; }
.table-grid { display: grid; grid-template-columns: 1.35fr 0.85fr; gap: 12px; }
.toolbar { margin-bottom: 8px; }
.alert-list { list-style: none; margin: 0 0 12px; padding: 0; display: flex; flex-direction: column; gap: 10px; }
.alert-list li { display: grid; grid-template-columns: 8px 1fr; gap: 8px; }
.dot { width: 8px; height: 8px; border-radius: 50%; margin-top: 5px; }
.dot-ok { background: #10b981; } .dot-warn { background: #f59e0b; } .dot-risk { background: #ef4444; }
.alert-list p { margin: 0; color: #1f2937; font-size: 12px; font-weight: 600; }
.alert-list small { color: #6b7280; font-size: 11px; line-height: 1.4; }
@media (max-width: 1200px) { .kpi-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } .main-grid,.table-grid { grid-template-columns: 1fr; } }
