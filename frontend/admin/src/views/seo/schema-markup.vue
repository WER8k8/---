...
<template>
  <YdPage title="Schema标记管理" subtitle="生成与管理结构化数据标记" surface="elevated">
    <YdHonestDataBanner
      level="mock"
      title="示意页 · 部分能力未接满"
      description="本页 JSON-LD 生成器可预览结构；已保存列表依赖后端 SEO 接口。对外勿将此处数据当作站点已上线结构化标记。"
    />
    <template #actions>
        ...
        <div>
          <div class="flex items-center justify-between mb-4">
            ...
            >
              ...
              <button
                class="px-3 py-1.5 text-sm text-green-600 bg-green-50 rounded-lg hover:bg-green-100 transition-colors flex items-center gap-1"
                @click="saveSchema"
              >
                <svg
                  class="w-4 h-4"
        ...
        <div class="p-4 overflow-auto max-h-[60vh]">
          <div class="bg-gray-50 rounded-lg p-4">
            <pre class="text-sm text-gray-700 whitespace-pre-wrap">{{
              JSON.stringify(viewModal.data?.content, null, 2)
            }}</pre>
          </div>
        </div>
      </div>
  ...
  switch (form.value.schemaType) {
    case 'Organization':
      ...
      fields.description = {
        type: 'textarea',
        label: '企业描述',
        placeholder: '专业轻集料混凝土生产商',
      };
      fields.url = {
        type: 'text',
        label: '官网地址',
        placeholder: 'https://www.youdingjiancai.com',
      };
      fields.logo = {
        type: 'text',
        label: 'Logo地址',
        placeholder: 'https://www.youdingjiancai.com/logo.png',
      };
    ...
    case 'Product':
      ...
      fields.description = {
        type: 'textarea',
        label: '产品描述',
        placeholder: '优质轻集料混凝土，密度800-1950kg/m³',
      };
    ...
    case 'Article':
      ...
      fields.description = {
        type: 'textarea',
        label: '文章摘要',
        placeholder: '本文介绍轻集料混凝土的技术特点和应用场景...',
      };
      fields.content = {
        type: 'textarea',
        label: '文章内容',
        placeholder: '轻集料混凝土是一种新型建筑材料...',
      };
...
const filteredSchemaList = computed(() => {
  ...
  return schemaList.value.filter(
    (item) =>
      item.name.toLowerCase().includes(query) || item.schema_type.toLowerCase().includes(query)
  );
});
...
const generateSchema = async () => {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.error('生成Schema失败:', error);
  } finally {
...
const validateSchema = async () => {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.error('验证Schema失败:', error);
  }
};
...
const saveSchema = async () => {
  if (!generatedSchema.value) return
  if (!schemaName.value) {
    schemaName.value = prompt('请输入Schema名称：') || ''
    if (!schemaName.value) return
  }
  try {
    const jsonLd = JSON.parse(generatedSchema.value)
    await seoAPI.schemaSave({
    ...
    if (import.meta.env.DEV) console.error('保存Schema失败:', error)
  }
}
...
const copyToClipboard = async () => {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.error('复制失败:', error);
  }
};
...
const refreshList = async () => {
  try {
    ...
    if (import.meta.env.DEV) console.error('获取Schema列表失败:', error);
  }
};
...
const exportSchema = async (item: (typeof schemaList.value)[0]) => {
  try {
    ...
    if (import.meta.env.DEV) console.error('导出Schema失败:', error);
  }
};
...
const deleteSchema = async (id: string) => {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.error('删除Schema失败:', error);
  }
};
...
const init = async () => {
  try {
    ...
    if (import.meta.env.DEV) console.error('获取Schema类型失败:', error);
  }
};
