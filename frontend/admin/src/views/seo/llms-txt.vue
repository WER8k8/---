...
async function fetchLLMSTxt() {
  try {
    const res = await fetch('/api/v1/seo/llms-txt', { headers: { Authorization: `Bearer ${getAuthToken()}` } })
    ...
    if (data.config) {
      if (data.config.business_type) form.value.business_type = data.config.business_type
      if (data.config.ai_model) form.value.ai_model = data.config.ai_model
      if (data.config.keywords) keywordsText.value = data.config.keywords.join('\n')
    }
  } catch { /* no saved content */ }
}

onMounted(() => { fetchLLMSTxt() })
...
async function validateContent() {
  ...
  try {
    const res = await seoAPI.validateLLMSTxt({ content: generatedContent.value });
    ...
    const issues = rawIssues.map((i) =>
      typeof i === 'string' ? i : `[${i.severity || 'note'}] ${i.message || ''}`
    );
    validationResult.value = {
      is_valid: !!payload.is_valid,
      issues,
      suggestions: payload.is_valid ? [] : ['请根据问题列表修订后再试'],
    };
  } catch (e: any) {
    validationResult.value = {
      is_valid: false,
...
async function saveContent() {
  ...
  try {
    const keywords = keywordsText.value.split('\n').filter((k) => k.trim());
    await fetch('/api/v1/seo/llms-txt', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getAuthToken()}` },
      body: JSON.stringify({ content: generatedContent.value, config: { ...form.value, keywords } }),
    })
