...
<template>
  <YdPage class="compliance-page" title="广告法合规审查" subtitle="自动检测广告法违禁词，确保内容合规" surface="elevated">
    ...
    <a-card
      ...
      class="history-card"
    >
      <div ref="historyTablePanelRef" class="yd-panel yd-table-panel">
        <div class="panel-head mb-3">
          <YdTableToolbar :loading="false" :target-ref="historyTablePanelRef" :show-export="false" @refresh="loadHistory" />
        </div>
        <YdDataTable
          :columns="historyColumns"
          :data-source="scanHistory"
          :pagination="{ pageSize: 10, total: scanHistory.length }"
          :table-props="{ size: tableSize, rowKey: 'id' }"
        >
          <template #bodyCell="{ column, record }">
    ...
    <a-modal v-model:open="historyDetailOpen" title="扫描详情" :footer="null" width="640">
      <a-descriptions v-if="historyDetailRecord" bordered size="small" :column="1">
        <a-descriptions-item label="扫描时间">{{ historyDetailRecord.scanned_at }}</a-descriptions-item>
        <a-descriptions-item label="类型">{{ historyDetailRecord.scan_type }}</a-descriptions-item>
        <a-descriptions-item label="违规数">{{ historyDetailRecord.violation_count }}</a-descriptions-item>
        <a-descriptions-item label="评分">{{ historyDetailRecord.score }}</a-descriptions-item>
        <a-descriptions-item label="状态">{{ historyDetailRecord.status === 'pass' ? '通过' : '违规' }}</a-descriptions-item>
      </a-descriptions>
    </a-modal>
  </YdPage>
</template>
...
const historyTablePanelRef = ref(null);
...
const startScan = async () => {
  ...
  try {
    ...
    scanResult.value = unwrapFetchedJson(await response.json());
...
const loadHistory = async () => {
  try {
    ...
    scanHistory.value = unwrapFetchedJson(await response.json());
  } catch (error) {
    if (import.meta.env.DEV) console.error('加载历史失败', error);
  }
};
