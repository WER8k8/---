...
<template>
  <YdPage title="EEAT 管理" subtitle="作者管理、EEAT评分与信任信号" surface="elevated">
    <YdHonestDataBanner
      level="mock"
      title="示意页 · 无独立深度审计"
      description="评分与信任信号来自页面内探测与示例数据，不可当作 E-E-A-T 合规结论。正式评估需结合内容与法务审核。"
    />
    <!-- 标签页切换 -->
    ...
    >
      ...
      >
        ...
        <div class="text-center mb-6">
          <div
            class="inline-flex items-center justify-center w-32 h-32 rounded-full bg-gradient-to-br from-blue-500 to-blue-600"
          >
...
const loadAuthors = async () => {
  try {
    ...
    if (import.meta.env.DEV) console.error('获取作者列表失败:', error);
  }
};
...
const loadTrustSignals = async () => {
  try {
    ...
    if (import.meta.env.DEV) console.error('获取信任信号失败:', error);
  }
};

const addAuthor = async () => {
  try {
    const expertiseAreas = authorForm.value.expertise
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    ...
    if (import.meta.env.DEV) console.error('添加作者失败:', error);
  }
};
...
const updateAuthor = async () => {
  try {
    ...
    if (import.meta.env.DEV) console.error('更新作者失败:', error);
  }
};
...
const deleteAuthor = async (id: string) => {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.error('删除作者失败:', error);
  }
};
...
const calculateScore = async () => {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.error('计算评分失败:', error);
  } finally {
