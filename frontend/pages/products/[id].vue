<template>
  <div class="product-detail-page">
    ...
    <div v-if="loading" class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-8 sm:py-12">
      <div class="animate-pulse">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          <div class="h-64 sm:h-80 lg:h-96 bg-gray-200 rounded-2xl"></div>
          <div class="space-y-4">
    ...
    <!-- Product Detail -->
    <div v-else-if="product" class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-8 sm:py-12">
      <!-- Breadcrumb -->
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
        ...
        <!-- Product Info -->
        <div>
          <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-text-primary mb-3 sm:mb-4">{{ product.name }}</h1>
          <p class="text-sm sm:text-base text-text-secondary mb-4 sm:mb-6">{{ product.description }}</p>

          <!-- Price -->
          <div v-if="product.price" class="mb-4 sm:mb-6">
          ...
          <div class="mb-4 sm:mb-6">
            ...
            <span v-if="product.stock > 0" class="text-sm text-text-secondary ml-2">{{ $t('product.stockCount', { count: product.stock }) }}</span>
          </div>

          <!-- Action Buttons -->
          <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-6 sm:mb-8">
            <button @click="openInquiryDialog" class="btn-primary btn-primary-lg flex-1">
              {{ $t('product.requestInquiry') }}
            </button>
            <button @click="openQuoteDialog" class="btn-outline flex-1">
              {{ $t('product.requestQuote') }}
            </button>
          </div>

          <!-- IM Contact Buttons -->
          <div v-if="merchantIM" class="border-t border-border pt-4 sm:pt-6">
            <h3 class="text-sm font-semibold text-text-primary mb-3">{{ $t('product.contactSupplier') }}</h3>
      ...
      <div v-if="product.specs" class="mb-8 sm:mb-12">
        <h2 class="text-xl sm:text-2xl font-bold text-text-primary mb-4 sm:mb-6">{{ $t('product.specifications') }}</h2>
        <div class="bg-surface rounded-2xl overflow-hidden">
          <table class="w-full">
            <tbody>
              <tr v-for="(value, key) in product.specs" :key="key" class="border-b border-border last:border-b-0">
                <td class="px-4 py-3 sm:px-6 sm:py-4 bg-surface-elevated font-medium text-text-primary w-1/3 sm:w-1/4">{{ key }}</td>
                <td class="px-4 py-3 sm:px-6 sm:py-4 text-text-secondary">{{ value }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <!-- Related Products -->
      <div v-if="relatedProducts && relatedProducts.length > 0" class="mb-8 sm:mb-12">
        <h2 class="text-xl sm:text-2xl font-bold text-text-primary mb-4 sm:mb-6">{{ $t('product.relatedProducts') }}</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          >
            <img :src="related.image || '/images/placeholder.jpg'" :alt="related.name" class="w-full h-32 sm:h-40 object-cover rounded-lg mb-3 sm:mb-4" />
            <h3 class="text-base sm:text-lg font-semibold text-text-primary group-hover:text-primary transition-colors">{{ related.name }}</h3>
            <p class="text-sm text-text-secondary mt-1">{{ related.price ? `$${related.price}` : '' }}</p>
          </NuxtLink>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6">
          <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-text-primary">{{ $t('product.requestInquiry') }}</h2>
            <button @click="showInquiryModal = false" class="text-text-secondary hover:text-text-primary">
    ...
    <div v-if="showQuoteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6">
          <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-text-primary">{{ $t('product.requestQuote') }}</h2>
            <button @click="showQuoteModal = false" class="text-text-secondary hover:text-text-primary">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          <form @submit.prevent="submitQuote" class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-text-primary mb-1">{{ $t('common.name') }}</label>
              <input v-model="quoteForm.name" type="text" required
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary" />
            </div>
            <div>
              <label class="block text-sm font-medium text-text-primary mb-1">{{ $t('common.email') }}</label>
              <input v-model="quoteForm.email" type="email" required
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary" />
            </div>
            <div>
              <label class="block text-sm font-medium text-text-primary mb-1">{{ $t('common.phone') }}</label>
            ...
            <div>
              <label class="block text-sm font-medium text-text-primary mb-1">{{ $t('common.company') }}</label>
              <input v-model="quoteForm.company" type="text"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary" />
            </div>
            <div>
              <label class="block text-sm font-medium text-text-primary mb-1">{{ $t('product.specifications') }}</label>
              <textarea v-model="quoteForm.productSpecs" rows="3"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                :placeholder="$t('product.specsPlaceholder')"></textarea>
            </div>
            <div>
              <label class="block text-sm font-medium text-text-primary mb-1">{{ $t('product.targetPrice') }}</label>
              <input v-model="quoteForm.targetPrice" type="text"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                placeholder="$" />
            </div>
            <div>
              <label class="block text-sm font-medium text-text-primary mb-1">{{ $t('common.quantity') }}</label>
              <input v-model.number="quoteForm.quantity" type="number" min="1" required
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary" />
            </div>
            <div class="flex gap-4">
              <button type="submit" class="btn-primary flex-1" :disabled="submittingQuote">
