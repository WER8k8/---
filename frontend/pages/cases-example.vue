...
const caseStudy: Case = {
  ...
  title: '上海某商业综合体保温工程',
  description:
    '为5万平方米的商业综合体提供外墙保温解决方案，采用模块化安装工艺，有效解决高层建筑保温施工难题。项目完成后节能效率提升45%，获得客户高度评价。',
  category: '商业建筑',
...
// SEO Configuration
const pageTitle = computed(() => `${caseStudy.title} - 保温工程案例 | ${SITE_CONFIG.name}`);
const pageDescription = computed(() => caseStudy.description || '');
const pageKeywords = computed(() =>
  [caseStudy.title, caseStudy.category || '', '保温工程', '施工案例', '建筑节能'].filter(Boolean)
);

const canonicalUrl = `${SITE_CONFIG.url}/cases/${caseStudy.id}`;
