...
const product: Product = {
  ...
  description:
    '采用最新纳米技术，导热系数低至0.023W/(m·K)，防火A级，防潮防水，适用于建筑外墙保温、冷库建设等场景。通过ISO 9001和CE认证，品质保证。',
  price: 188,
...
// SEO Configuration
const pageTitle = computed(() => `${product.name} - 技术参数与报价 | ${SITE_CONFIG.name}`);
const pageDescription = computed(() => product.description || '');
const pageKeywords = computed(() =>
  [product.name, product.category || '', '保温材料', '建筑节能', ...(product.tags || [])].filter(
    Boolean
  )
);

const canonicalUrl = `${SITE_CONFIG.url}/products/${product.id}`;
