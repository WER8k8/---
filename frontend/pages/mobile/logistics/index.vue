  ...
  <div class="min-h-screen bg-gray-50 pb-safe-bottom">
    ...
    <MobileNavbar safe-area-top :blur="true">
      <div class="flex items-center h-14">
        ...
        <h1 class="ml-2 text-base font-semibold text-gray-900">{{ $t('mobile.logistics.title') }}</h1>
      </div>
    </MobileNavbar>
    ...
    <!-- Loading State -->
    <div v-if="pending" class="px-4 py-6">
    ...
    <!-- Tracking Result -->
    <div v-else-if="trackingResult" class="px-4 pb-4">
      <!-- Order Info -->
      <div class="bg-white rounded-2xl p-4 shadow-sm mb-4">
        <div class="flex items-center justify-between mb-3">
          <h2 class="text-sm font-semibold text-gray-900">{{ $t('mobile.logistics.orderInfo') }}</h2>
          <span
            :class="[
      ...
      <!-- Tracking Timeline -->
      <div class="bg-white rounded-2xl p-4 shadow-sm">
        <h2 class="text-sm font-semibold text-gray-900 mb-4">{{ $t('mobile.logistics.timeline') }}</h2>
        <div class="space-y-4">
          <div
          ...
          >
            ...
            <div class="flex flex-col items-center">
              ...
              ></div>
              <div v-if="idx < trackingResult.events.length - 1" class="w-0.5 h-full bg-gray-200 mt-1"></div>
            </div>
            <!-- Event Content -->
            <div class="flex-1 pb-4">
              <p class="text-sm font-medium text-gray-900">{{ event.description }}</p>
              <p class="text-xs text-gray-500 mt-1">{{ event.location }}</p>
              <p class="text-xs text-gray-400 mt-0.5">{{ new Date(event.timestamp).toLocaleString() }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else-if="!trackingNumber && !trackingResult" class="px-4 py-20 text-center">
      <svg class="w-16 h-16 mx-auto text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
      </svg>
      <h3 class="text-base font-medium text-gray-900 mb-2">{{ $t('mobile.logistics.noTracking') }}</h3>
      <p class="text-sm text-gray-500">{{ $t('mobile.logistics.noTrackingDesc') }}</p>
    </div>

    <!-- Not Found State -->
    <div v-else-if="trackingSearched && !trackingResult" class="px-4 py-20 text-center">
      <svg class="w-16 h-16 mx-auto text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <h3 class="text-base font-medium text-gray-900 mb-2">{{ $t('mobile.logistics.notFound') }}</h3>
      <p class="text-sm text-gray-500">{{ $t('mobile.logistics.notFoundDesc') }}</p>
    </div>
