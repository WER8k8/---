...
<template>
  <div class="min-h-screen bg-gray-50 pb-safe-bottom">
    ...
    <MobileNavbar safe-area-top :blur="true">
      <template #default>
        <div class="flex items-center justify-between h-14">
          <h1 class="text-base font-semibold text-gray-900">{{ t('mobile.alerts.title') }}</h1>
          <button
    ...
    <div v-else-if="error" class="px-4 py-20 text-center">
      ...
      <h3 class="text-base font-medium text-gray-900 mb-2">{{ t('mobile.alerts.loadFailed') }}</h3>
      <p class="text-sm text-gray-500 mb-6">{{ error }}</p>
      <button class="mobile-btn-primary px-6 py-3 rounded-full" @click="fetchAlerts">
        {{ t('mobile.alerts.retry') }}
      </button>
    </div>

    <!-- Empty State -->
    <div v-else-if="alerts.length === 0" class="px-4 py-20 text-center">
      <svg class="w-16 h-16 mx-auto text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
      </svg>
      <h3 class="text-base font-medium text-gray-900 mb-2">{{ t('mobile.alerts.noAlerts') }}</h3>
      <p class="text-sm text-gray-500 mb-6">{{ t('mobile.alerts.noAlertsDesc') }}</p>
    </div>

    <!-- Alerts List -->
    <div v-else class="px-4 py-2 space-y-2 pb-20">
      <div
      ...
      >
        <div class="flex items-start justify-between mb-2">
          <h3 class="text-sm font-semibold text-gray-900 flex-1 truncate">{{ alert.title }}</h3>
          <span
            :class="[
        ...
        <p class="text-xs text-gray-500 mb-2 line-clamp-2">{{ alert.description }}</p>
    ...
    <!-- Create Alert Modal -->
    <div v-if="showCreateAlert" class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-end justify-center">
      <div class="bg-white w-full max-h-[90vh] rounded-t-2xl overflow-y-auto">
        <div class="p-5">
          <div class="flex items-center justify-between mb-5">
            <h2 class="text-lg font-bold text-gray-900">{{ t('mobile.alerts.createAlert') }}</h2>
            <button @click="showCreateAlert = false" class="p-1">
