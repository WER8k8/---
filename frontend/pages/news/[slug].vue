  ...
  <div class="news-detail-page">
    ...
    >
        ...
        <div class="absolute bottom-0 left-0 right-0 p-8 md:p-16">
          <div class="max-w-4xl mx-auto">
            <div class="flex flex-wrap gap-2 mb-4">
              ...
              <span
                v-if="article.author"
                class="px-3 py-1 bg-white/20 text-white text-sm font-medium rounded-full"
              >
                {{ article.author }}
              </span>
            </div>
            ...
            <p
              ...
              class="text-xl text-white/80 mb-4"
            >
              {{ article.subtitle }}
            </p>
            <div class="flex flex-wrap gap-4 text-white/70 text-sm">
              ...
              <span class="flex items-center gap-1">
                ...
                {{ article.view_count }} {{ t('news.viewCount') }}
              </span>
            </div>
          </div>
        </div>
      ...
      <section class="article-content bg-surface py-12 md:py-16">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <NuxtLink
              to="/news"
              class="text-accent hover:text-accent/80 flex items-center gap-2"
            >
              <svg
                class="w-5 h-5"
                fill="none"
            ...
            <div class="flex items-center gap-4">
              <span class="text-text-secondary text-sm">{{ t('news.detail.share') }}</span>
              <button
                class="p-2 rounded-full bg-surface-elevated hover:bg-accent/10 transition-colors"
                :title="t('news.detail.wechatShare')"
              >
                <svg
              ...
              <button
                class="p-2 rounded-full bg-surface-elevated hover:bg-accent/10 transition-colors"
                :title="t('news.detail.weiboShare')"
              >
                <svg
                  class="w-5 h-5 text-text-secondary"
                  fill="currentColor"
      ...
      <section
        ...
        class="related-articles py-12 md:py-16 bg-surface-elevated"
      >
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              <div
                class="aspect-[16/9] overflow-hidden bg-gradient-to-br from-accent/10 to-primary/10"
              >
                <img
              ...
              <div class="p-4">
                <h3
                  class="text-base font-semibold text-text-primary line-clamp-2 group-hover:text-accent transition-colors"
                >
                  {{ related.title }}
                </h3>
...
useHead({
  title: computed(() =>
    article.value ? `${article.value.title} - ${t('news.seo.detailTitle')}` : t('news.seo.detailFallbackTitle')
  ),
  meta: [
    ...
    { property: 'og:url', content: computed(() => `${SITE_CONFIG.url}/news/${slug}`) },
    {
      property: 'og:image',
      content: computed(
        () => article.value?.cover_image || `${SITE_CONFIG.url}/images/news-default.jpg`
      ),
    },
  ],
});
