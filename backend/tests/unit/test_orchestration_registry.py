...
def _registry() -> SlotRegistry:
    return SlotRegistry(
        [
            GoodJobExecutor(base_url="http://goodjob.test"),
...
def test_registry_returns_adapter_by_archetype():
    ...
    assert isinstance(
        registry.slot_adapter(SlotArchetype.EXECUTOR), GoodJobExecutor
    )
    assert isinstance(
        registry.slot_adapter(SlotArchetype.ERP_BACKEND), ErpGatewayAdapter
    )
