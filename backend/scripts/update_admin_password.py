...
def update_admin_password():
    ...
    parser.add_argument(
        "--password",
        help="新密码（如果未提供，将从环境变量 ADMIN_PASSWORD 读取）")
    ...
    if not new_password:
        logger.info('错误: 未提供密码。请使用 --password 参数、--interactive 参数或设置 ADMIN_PASSWORD 环境变量')
        return False

    # 验证密码强度
    is_valid, errors = validate_password_strength(new_password)
    if not is_valid:
        logger.info('密码强度不符合要求:')
        for error in errors:
