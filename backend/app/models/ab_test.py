...
class ABTest(Base):
    ...
    experiment_type = Column(
        String(50),
        default="page",
        comment="实验类型: page/component/element")
    ...
    primary_metric = Column(
        String(100),
        comment="主要指标: conversion/click_through/time_on_page")
    ...
    updated_at = Column(
        ...
        comment="更新时间")
    created_by = Column(UUID_TYPE, ForeignKey("users.id"), comment="创建人ID")
    # 关系
...
class ABTestEvent(Base):
    ...
    event_type = Column(
        String(50),
        nullable=False,
        comment="事件类型: impression/click/conversion/scroll")
...
class ABTestConversion(Base):
    ...
    conversion_type = Column(
        String(50),
        nullable=False,
        comment="转化类型: form_submit/click/purchase")
