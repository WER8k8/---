...
class EmailService:
    ...
    def send_inquiry_notification(self, inquiry_data: dict) -> bool:
        ...
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background: linear-gradient(135deg, #4a9b8c 0%, #2a6b60 100%); padding: 30px; color: white;">
    ...
    def send_verification_code(self, to_email: str, code: str) -> bool:
        """发送管理端邮箱登录验证码。"""
        if not self.enabled:
            logger.warning("邮件服务未配置，跳过验证码邮件")
            return False
        subject = "优丁建材 - 登录验证码"
        html_content = f"""
    ...
    def send_email(self, to: str, subject: str, html: str) -> bool:
        """兼容函数式调用或实例调用 send_email。"""
        return self.send_raw(to, subject, html)

    async def send_batch(
        self,
        tenant_id: str,
        template: str = "b2b_outreach",
        language: str = "en",
        ...
        **kwargs: Any,
        ...
        sent_count = len(recipients) if recipients else 1
        return {
            "tenant_id": tenant_id,
            "template": template,
            "language": language,
    ...
    def smtp_status(self) -> dict:
        ...
        return {
            ...
            "server": self.smtp_server or None,
            "port": self.smtp_port,
            "username_set": bool(self.smtp_username),
            "from_email": self.from_email or self.smtp_username or None,
            "production_ready": self.enabled,
        }


email_service = EmailService()

# 兼容函数式导入：from app.services.email_service import send_email
def send_email(to: str, subject: str, html: str) -> bool:
    return email_service.send_raw(to, subject, html)
