...
class SeoAnalyzer:
    ...
    def get_dashboard(self) -> dict:
        ...
        avg_rank_result = (
            self.db.query(func.avg(Keyword.current_ranking))
            .filter(Keyword.is_active, Keyword.current_ranking.isnot(None))
            .scalar()
        )
        avg_rank = round(
            float(avg_rank_result),
            1) if avg_rank_result else None

        recent_audits = (
            self.db.query(
                func.count(
                    SiteAudit.id)) .filter(
                SiteAudit.created_at >= datetime.now(
                    timezone.utc) -
                timedelta(
                    days=30)) .scalar() or 0)

        last_audit = self.db.query(SiteAudit).order_by(
        ...
        ai_optimized_pages = (
            self.db.query(
                ...
                AiOptimizationLog.created_at >= datetime.now(
                    timezone.utc) -
                timedelta(
                    days=30)) .scalar() or 0)

        llms_config_exists = (
            self.db.query(func.count(ContentPage.id))
            .filter(
                ContentPage.slug == "llms-txt",
        ...
        keyword_trend_raw = (
            self.db.query(
                func.date(
                    KeywordRanking.last_checked_at), func.avg(
                    KeywordRanking.current_position))
            .filter(KeywordRanking.last_checked_at.isnot(None))  # 过滤NULL值
    ...
    def analyze_keyword_rankings(self, keyword_id: str) -> dict:
        ...
        for r in reversed(rankings):
            trend.append({"date": r.checked_at.strftime("%Y-%m-%d"),
                         "rank": r.ranking, "engine": r.search_engine})

        rank_change = None
        if len(rankings) >= 2:
            rank_change = (rankings[0].ranking or 0) - \
                (rankings[-1].ranking or 0)
    ...
    def get_keyword_groups(self) -> list[dict]:
        ...
        :return: 返回处理结果。
        """
        keywords = self.db.query(Keyword).filter(Keyword.is_active).all()
        groups = {}
        for kw in keywords:
    ...
    def get_seo_pages_summary(self) -> list[dict]:
        ...
        pages = self.db.query(ContentPage).filter(ContentPage.is_active).all()
        results = []
        for p in pages:
            seo = (
                self.db.query(SeoMetadata)
                .filter(
                    SeoMetadata.resource_id == str(p.id),
                    SeoMetadata.resource_type.in_(("content_page", "page")),
                )
                .first()
            )
            ...
            results.append(
                {
                    ...
                    "view_count": getattr(
                        ...
                        "view_count",
                        0) or 0,
                    "optimized_at": seo.updated_at.isoformat() if seo and seo.updated_at else None,
