...
class SchemaGenerator:
    ...
    def generate_schema(self,
                        ...
                        data: Dict[str,
                                   Any],
                        include_context: bool = True) -> Dict[str,
                                                              Any]:
        """
        生成指定类型的Schema标记
        ...
        generator_method = getattr(
            self, f"_generate_{schema_type.lower()}", None)
    ...
    def _generate_organization(
            self, data: Dict[str, Any], include_context: bool) -> Dict[str, Any]:
        ...
        if include_context:
            schema["sameAs"] = data.get(
                "social_links", [
                    "https://www.weixin.com/youding", "https://www.linkedin.com/company/youding"])
    ...
    def _generate_webpage(
        ...
        if include_context and data.get("breadcrumb"):
            schema["breadcrumb"] = self._generate_breadcrumb(
                {"items": data["breadcrumb"]}, False)

        return schema

    def _generate_breadcrumblist(
            self, data: Dict[str, Any], include_context: bool) -> Dict[str, Any]:
        ...
        return {
            ...
            "itemReviewed": {
                ...
                "name": data.get(
                    "product_name",
                    "")},
            "datePublished": data.get(
                "published_date",
                ""),
        }

    def _generate_aggregaterating(
            self, data: Dict[str, Any], include_context: bool) -> Dict[str, Any]:
    ...
    def validate_schema(self, content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "@context" not in content:
            errors.append({"code": "missing_context",
                          "message": "缺少@context字段"})
        elif content["@context"] != "https://schema.org":
            warnings.append({"code": "invalid_context",
                             "message": "@context建议使用https://schema.org"})
        ...
        if "@type" not in content:
            ...
            if schema_type not in self.SUPPORTED_SCHEMA_TYPES:
                warnings.append({"code": "unsupported_type",
                                 "message": f"不支持的Schema类型: {schema_type}"})

        # 特定类型验证
        if schema_type == "Product":
            if not content.get("name"):
                errors.append({"code": "product_missing_name",
                              "message": "Product缺少name字段"})

        if schema_type == "Article":
            if not content.get("headline"):
                errors.append({"code": "article_missing_headline",
                              "message": "Article缺少headline字段"})
            if not content.get("articleBody"):
                errors.append({"code": "article_missing_body",
                              "message": "Article缺少articleBody字段"})

        if schema_type == "Organization":
            if not content.get("name"):
                errors.append({"code": "org_missing_name",
