...
class SiteAuditEngine:
    ...
    def _check_technical_compliance(self, page_data: dict) -> list[dict]:
        """_check_technical_compliance。

        参数说明：
        :param self: 参数 self
        :param page_data: 参数 page_data
        :return: 返回处理结果。
        """
        issues = []
                ...
                issues.append(
                    {
                        ...
                        "message": f"密度值{val} kg/m³超出合规范围(800-1950)",
                    }
                )
                ...
                issues.append(
                    {
                        "severity": "medium",
                        "dimension": "technical_compliance",
                        "message": f"导热系数{val}超出合规范围(0.18-0.50)",
                    }
                )
    ...
    def _check_meta_tags(self, page_data: dict) -> list[dict]:
        ...
        if not meta_title:
            ...
            issues.append({"severity": "medium",
                           "dimension": "meta_tags",
                           "message": f"Meta标题过短({len(meta_title)}字)"})
        elif len(meta_title) > 100:
            issues.append({"severity": "medium",
                           "dimension": "meta_tags",
                           "message": f"Meta标题过长({len(meta_title)}字)"})
            ...
            issues.append(
                {
                    "severity": "medium",
                    "dimension": "page_structure",
                    "message": f"页面包含多个H1标签({len(h1_tags)}个)",
                }
            )
    ...
    def _check_security_compliance(self, page_data: dict) -> list[dict]:
        ...
        if re.search(
            r"(password|secret|token|key|auth).*?=",
            html,
                re.IGNORECASE):
            issues.append({"severity": "high",
                           "dimension": "security_compliance",
    ...
    def _check_ai_crawler_friendly(self, page_data: dict) -> list[dict]:
        ...
        if not re.search(r'<meta\s+name=["\']robots["\']', html):
            issues.append({"severity": "medium",
                           "dimension": "ai_crawler_friendly",
                           "message": "缺少robots meta标签"})

        if not re.search(r'<link[^>]*rel=["\']canonical["\']', html):
            issues.append({"severity": "medium",
                           "dimension": "ai_crawler_friendly",
                           "message": "缺少规范URL(canonical)标记"})
        ...
        if not re.search(
            ...
            issues.append({"severity": "low",
                           "dimension": "ai_crawler_friendly",
                           "message": "缺少JSON-LD结构化数据"})

        if not re.search(
            r'<link[^>]*rel=["\']sitemap["\']',
                html) and "/sitemap.xml" not in html:
            issues.append({"severity": "low",
                           "dimension": "ai_crawler_friendly",
                           "message": "未引用sitemap.xml"})
    ...
    def _check_mobile_friendly(self, page_data: dict) -> list[dict]:
        ...
        if not re.search(r'<meta\s+name=["\']viewport["\']', html):
            issues.append({"severity": "high",
                           "dimension": "mobile_friendly",
                           "message": "缺少viewport视口设置"})

        if not re.search(
                r"@media\s*[\(]",
                html) and not re.search(
                r'<link[^>]+media=["\']',
                html):
            issues.append(
                {
                ...
                issues.append(
                    {
                        "severity": "low",
                        "dimension": "mobile_friendly",
                        "message": f"字体大小{size}px过小，移动端阅读困难",
                    }
                )
            ...
            issues.append(
                {
                    "severity": "low",
                    "dimension": "mobile_friendly",
                    "message": "页面含表格但无viewport设置，移动端可能错位",
                }
            )
            ...
            issues.append(
                {
                    "severity": "low",
                    "dimension": "mobile_friendly",
                    "message": "缺少format-detection设置，电话号码可能被错误识别",
                }
            )
            ...
            issues.append(
                {
                    ...
                    "message": f"内联样式过多({len(inline_styles)}处)，建议合并到CSS文件",
                }
            )
            ...
            issues.append(
                {
                    "severity": "medium",
                    "dimension": "performance",
                    "message": f"外部资源过多({total_resources}个)，建议合并减少请求数",
                }
            )

        async_scripts = re.findall(
            r'<script[^>]*(?:async|defer)[^>]*src=["\']([^"\']+)["\']', html)
            ...
            issues.append(
                {
                    "severity": "low",
                    "dimension": "performance",
                    "message": f"同步阻塞脚本过多({blocking_scripts}个)，建议使用async/defer",
                }
            )

        large_images = re.findall(
            r'<img[^>]*src=["\']([^"\']+\.(?:jpg|jpeg|png|gif))["\']', html)
        if len(large_images) > 15:
            issues.append(
                {
                    "severity": "low",
                    "dimension": "performance",
                    "message": f"图片过多({len(large_images)}张)，建议使用懒加载",
                }
            )

        if re.search(
            r'<link[^>]*rel=["\']stylesheet["\'][^>]*media=["\']print["\']',
                html):
            issues.append(
                {
                    "severity": "low",
                    "dimension": "performance",
                    "message": "存在打印样式表，建议使用media=print限制加载",
                }
            )
            ...
            issues.append(
                {
                    "severity": "medium",
                    "dimension": "performance",
                    "message": f"页面体积过大({total_size // 1000}KB)，建议优化",
                }
            )
    ...
    def _check_link_quality(self, page_data: dict) -> list[dict]:
        ...
        external_links = re.findall(
            r'<a[^>]*href=["\']https?://(?!.*youding\.com)[^"\']+["\']', html)
        nofollow_links = re.findall(
            r'<a[^>]*rel=["\'][^"\']*nofollow[^"\']*["\']', html)
            ...
            issues.append(
                {
                    "severity": "low",
                    "dimension": "link_quality",
                    "message": f"站内链接过少({len(internal_links)}个)，建议增加相关页面互链",
                }
            )

        if external_links:
            nofollow_external = re.findall(
                r'<a[^>]*href=["\']https?://(?!.*youding\.com)[^"\']+["\'][^>]*rel=["\'][^"\']*nofollow[^"\']*["\']',
                html,
            )
            if len(nofollow_external) < len(external_links):
                issues.append({"severity": "medium",
                               "dimension": "link_quality",
                               "message": f"外部链接({len(external_links)}个)未全部使用nofollow",
                               })

        anchor_text_missing = re.findall(
            r'<a[^>]*href=["\'][^"\']+["\'][^>]*>\s*</a>', html)
            ...
            issues.append(
                {
                    "severity": "medium",
                    "dimension": "link_quality",
                    "message": f"存在空锚文本链接({len(anchor_text_missing)}个)",
                }
            )
            ...
            issues.append(
                {
                    "severity": "low",
                    "dimension": "link_quality",
                    "message": f"页内锚点过多({len(broken_anchors)}个)，可能影响爬虫抓取效率",
                }
            )
    ...
    def _check_content_quality(self, page_data: dict) -> list[dict]:
            ...
            issues.append(
                {
                    ...
                    "message": f"正文内容过少({word_count}字)，建议至少300字",
                }
            )
        elif word_count < 600:
            issues.append(
                {
                    "severity": "medium",
                    "dimension": "content_quality",
                    "message": f"正文偏少({word_count}字)，建议增至600字以上",
                }
            )
            ...
            issues.append(
                {
                    "severity": "medium",
                    "dimension": "content_quality",
                    "message": f"段落过少({len(non_empty_paras)}段)，建议分段提高可读性",
                }
            )
        ...
        if dup_count > 3:
            issues.append({"severity": "low",
                           "dimension": "content_quality",
                           "message": f"存在重复段落({dup_count}处)，建议去重"})
            ...
            issues.append(
                {
                    ...
                    "message": "缺少JSON-LD结构化数据，影响AI搜索引擎理解页面",
                }
            )
        else:
                        ...
                        issues.append(
                            {
                                "severity": "low",
                                "dimension": "ai_search_optimization",
                                "message": f"结构化数据类型'{sd_type}'非推荐类型",
                            }
                        )
            ...
            issues.append(
                {
                    "severity": "high",
                    "dimension": "ai_search_optimization",
                    "message": "缺少Meta Description，AI搜索引擎无法获取页面摘要",
                }
            )
            ...
            issues.append(
                {
                    "severity": "high",
                    "dimension": "ai_search_optimization",
                    "message": "缺少Title标签，影响AI搜索引擎理解页面主题",
                }
            )
            ...
            issues.append(
                {
                    "severity": "medium",
                    "dimension": "ai_search_optimization",
                    "message": "页面被设置为noindex，AI搜索引擎将不会索引此页面",
                }
            )
            ...
            issues.append(
                {
                    "severity": "medium",
                    "dimension": "ai_search_optimization",
                    "message": f"内容过短({content_length}字)，AI搜索引擎难以准确理解主题",
                }
            )
    ...
    def run_basic_audit(self, page_data: dict) -> dict:
        ...
        for issue in sorted(
            ...
            recommendations.append(
                {"issue": issue["message"], "priority": issue["severity"]})
...
按优先级输出5条建议，每行一个。"""
            ...
            ai_recommendations = [
                line.strip("- ").strip() for line in ai_content.split("\n") if line.strip()]
            basic_result["recommendations"].extend(
                {"issue": rec, "priority": "medium", "source": "ai"} for rec in ai_recommendations[:10]
            )
    ...
    async def run_audit(self, url: str, audit_type: str = "full") -> dict:
        ...
        :param self: 参数 self
        :param url: 参数 url
        :param audit_type: 参数 audit_type
        :return: 返回处理结果。
        """
        if audit_type == "quick":
