...
from app.services.foreign_trade.osint.constants import http_get
...
def _resolve_cache_dir() -> Path:
    ...
    import os
    import tempfile
    custom = os.environ.get("FOREIGN_TRADE_CACHE_DIR", "").strip()
    if custom:
        return Path(custom) / "sanctions"
    return Path(tempfile.gettempdir()) / "shangxian_foreign_trade" / "sanctions"
...
def _check_sanctions_extracted():
    ...
    return exact_match_found, name_normalized, hits, name_upper
