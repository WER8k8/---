...
class AppException(Exception):
    def __init__(
        ...
        :param status_code: 参数 status_code
        :param data: 参数 data
        :return: 返回处理结果。
        """
        self.code = code
...
async def not_configured_exception_handler(
    ...
    """not_configured_exception_handler。
    ...
    """
    logger.warning(
        "NotConfiguredError: code=%s message=%s path=%s",
        exc.code,
        str(exc),
        request.url.path,
    )
...
async def fake_delivery_exception_handler(
    request: Request, exc: FakeDeliveryViolation
) -> JSONResponse:
    """fake_delivery_exception_handler。
    ...
    """
    logger.error(
        "FakeDeliveryViolation: code=%s path=%s violations=%s",
        exc.code,
        request.url.path,
        exc.violations[:3],
    )
...
async def sqlalchemy_exception_handler(
        request: Request,
        exc: SQLAlchemyError) -> JSONResponse:
...
def register_exception_handlers(app):
    ...
    :return: 返回处理结果。
    """
    app.add_exception_handler(NotConfiguredError, not_configured_exception_handler)
    app.add_exception_handler(FakeDeliveryViolation, fake_delivery_exception_handler)
