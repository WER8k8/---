...
def export_user_gdpr_data(user_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def anonymize_user(req: AnonymizeRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    svc = GDPRComplianceService(db)
