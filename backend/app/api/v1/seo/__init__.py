...
from app.schemas.seo import (KeywordCreate, KeywordResponse, KeywordUpdate,
                             LlmsConfigCreate, LlmsConfigResponse,
                             LlmsConfigUpdate, SiteAuditCreate,
                             SiteAuditResponse)
...
router.include_router(
    ...
    tags=["seo-audit"])
router.include_router(
    schema_markup_router,
    prefix="/schema-markup",
    tags=["seo-schema"])
...
router.include_router(report_export_router, tags=["seo-report"])
...
def list_llms_configs(db: Session = Depends(get_db)):
    """获取LLM配置列表"""
    return db.query(LlmsConfig).filter(LlmsConfig.is_active).all()


@router.post("/llms", response_model=LlmsConfigResponse)
def create_llms_config(
        req: LlmsConfigCreate,
        db: Session = Depends(get_db),
        admin=Depends(require_admin)):
    """创建LLM配置"""
