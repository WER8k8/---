...
from app.core.security import get_current_user, optional_auth, require_admin
from app.models.schema_markup import SchemaMarkup, SchemaTemplate
from app.models.user import User
from app.schemas.schema_markup import (GenerateSchemaRequest,
                                       SchemaMarkupCreate,
...
def list_schema_markups(db: Session = Depends(get_db)):
    """获取所有Schema标记记录"""
    return db.query(SchemaMarkup).filter(
        SchemaMarkup.is_active).order_by(
        SchemaMarkup.created_at.desc()).all()


@router.get("/{markup_id}", response_model=SchemaMarkupResponse)
def get_schema_markup(markup_id: str, db: Session = Depends(get_db)):
    """获取单个Schema标记记录"""
    markup = db.query(SchemaMarkup).filter(
        SchemaMarkup.id == markup_id).first()
    if not markup:
...
def create_schema_markup(
    ...
    if not validation["is_valid"]:
        error_messages = "; ".join([e["message"]
                                   for e in validation["errors"]])
        raise HTTPException(status_code=400,
                            detail=f"Schema验证失败: {error_messages}")
...
def list_schema_templates(db: Session = Depends(get_db)):
    """获取所有Schema模板"""
    return db.query(SchemaTemplate).filter(SchemaTemplate.is_active).all()
