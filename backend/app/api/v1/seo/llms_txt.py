...
def generate_llms_txt(req: GenerateRequest, db: Session = Depends(get_db), current_user: User = Depends(optional_auth)):
    lines = []
...
def validate_llms_txt(req: ValidateRequest, current_user: User = Depends(optional_auth)):
    issues = []
    content = req.content
