...
def _fetch_llms_txt_data(db: Session):
    """查询产品、工程案例与内容页数据（供 LLMs.txt 拼装）。"""
    products = (
        db.query(Product)
        .options(
            load_only(
    ...
    cases = (
        ...
        .filter(CaseStudy.status.in_(["published", "active"]))
        .limit(10)
        .all()
    )
    if not cases:
        cases = (
            db.query(CaseStudy) .options(
                load_only(
                    CaseStudy.project_name,
                    CaseStudy.slug,
                    CaseStudy.description)) .filter(
                CaseStudy.is_active) .limit(10) .all())
    pages = (
        db.query(ContentPage) .options(
            load_only(
                ContentPage.title,
                ContentPage.slug,
                ContentPage.page_type,
                ContentPage.status)) .filter(
            ContentPage.is_active,
            ContentPage.status == "published") .limit(10) .all())
    return products, cases, pages


def _render_llms_txt_core(products, cases, pages) -> str:
    """按产品/案例/内容页拼装 LLMs.txt 文本。"""
...
优丁建材有限公司（Youding Building Materials Co., Ltd.）是一家专注于新型建筑材料研发与生产的企业，拥有20年行业经验。公司主营轻集料混凝土、陶粒混凝土、保温砂浆等产品，年产能力超百万吨，通过ISO9001质量管理体系认证。
...
"""
    ...
    for product in products:
        spec = product.specifications_text or (
            str(product.specifications) if product.specifications else "")
...
"""
    ...
    for case in cases:
        title = case.project_name or case.slug
...
- 详情: https://www.youdingjiancai.com/pages/{page.slug}
...
def build_llms_txt_from_db(db: Session) -> dict:
    """由数据库拼装 LLMs.txt（供 GET 接口与预生成脚本复用）。"""
    products, cases, pages = _fetch_llms_txt_data(db)
    llms_content = _render_llms_txt_core(products, cases, pages)
    return {
...
def get_llms_txt(db: Session = Depends(get_db)):
    ...
    :return: 返回处理结果。
    """
    return build_llms_txt_from_db(db)
