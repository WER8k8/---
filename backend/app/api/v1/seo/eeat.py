...
def create_author(data: Dict[str, Any], db: Session = Depends(
        get_db), token: str = Depends(oauth2_scheme)):
    """创建新作者/专家"""
    author = Author(
        id=str(uuid4()),
        name=data.get("name"),
...
def get_authors(
    ...
    if verified_only:
        query = query.filter(Author.is_verified)
...
def get_author(
    ...
    # 获取认证信息
    certifications = db.query(AuthorCertification).filter(
        AuthorCertification.author_id == author_id).all()
...
def update_author(author_id: str, data: Dict[str, Any], db: Session = Depends(
        get_db), token: str = Depends(oauth2_scheme)):
    """更新作者信息"""
    author = db.query(Author).filter(Author.id == author_id).first()
    if not author:
...
def delete_author(
    ...
    # 删除关联数据
    db.query(AuthorCertification).filter(
        AuthorCertification.author_id == author_id).delete()
    db.query(ArticleAuthor).filter(
        ArticleAuthor.author_id == author_id).delete()
...
@router.post("/authors/{author_id}/certifications",
             summary="添加认证", response_model=Dict)
def add_certification(author_id: str,
                      data: Dict[str,
                                 Any],
                      db: Session = Depends(get_db),
                      token: str = Depends(oauth2_scheme)):
    ...
    certification = AuthorCertification(
        ...
        expiration_date=datetime.fromisoformat(
                data.get("expiration_date")) if data.get("expiration_date") else None,
...
@router.delete("/certifications/{cert_id}",
               summary="删除认证", response_model=Dict)
def delete_certification(
        cert_id: str,
        db: Session = Depends(get_db),
        token: str = Depends(oauth2_scheme)):
    """删除认证"""
    cert = db.query(AuthorCertification).filter(
        AuthorCertification.id == cert_id).first()
    if not cert:
...
def calculate_eeat_score(data: Dict[str, Any], db: Session = Depends(
        get_db), token: str = Depends(oauth2_scheme)):
    """计算内容的EEAT分数"""
...
def get_eeat_score(
    ...
    """获取内容的EEAT评分记录"""
    scores = (
        db.query(EEATScore).filter(
            EEATScore.content_id == content_id).order_by(
            EEATScore.evaluated_at.desc()).all())
...
@router.post("/content/{content_id}/trust-signals",
             summary="添加信任信号", response_model=Dict)
def add_trust_signal(content_id: str,
                     data: Dict[str,
                                Any],
                     db: Session = Depends(get_db),
                     token: str = Depends(oauth2_scheme)):
...
@router.get("/content/{content_id}/trust-signals",
            summary="获取内容信任信号", response_model=Dict)
def get_content_trust_signals(
        content_id: str,
        db: Session = Depends(get_db),
        token: str = Depends(oauth2_scheme)):
    """获取内容的信任信号列表"""
    signals = db.query(TrustSignal).filter(
        TrustSignal.content_id == content_id).all()
...
@router.post("/articles/{article_id}/authors",
             summary="关联作者到文章", response_model=Dict)
def add_article_author(article_id: str,
                       data: Dict[str,
                                  Any],
                       db: Session = Depends(get_db),
                       token: str = Depends(oauth2_scheme)):
...
@router.delete("/articles/{article_id}/authors/{author_id}",
               summary="取消文章作者关联", response_model=Dict)
