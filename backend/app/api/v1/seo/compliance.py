...
def get_scan_result(
        ...
        token: str = Depends(oauth2_scheme)):
    """获取指定扫描结果的详细信息"""
    scan_result = db.query(ComplianceScanResult).filter(
        ComplianceScanResult.id == scan_id).first()
...
def get_keywords(
        ...
        severity: str = None):
    """获取广告法违禁词列表"""
    query = db.query(AdvertisementLawKeyword).filter(
...
def list_violations(
    ...
    violations = query.order_by(
        ComplianceViolation.created_at.desc()).offset(offset).limit(limit).all()
...
@router.post("/keywords", summary="添加违禁词",
             dependencies=[Depends(require_admin)])
def add_keyword(data: Dict[str, Any], db: Session = Depends(
    ...
    existing = db.query(AdvertisementLawKeyword).filter(
        AdvertisementLawKeyword.keyword == keyword).first()
    if existing:
    ...
    # 刷新缓存
    logger.info("违禁词缓存待刷新（新关键词已入库）: %s", keyword)
    return {
        "success": True,
        "message": "添加成功",
        "data": {
...
@router.put("/keywords/{keyword_id}", summary="更新违禁词",
            dependencies=[Depends(require_admin)])
def update_keyword(keyword_id: str,
    ...
    db.refresh(keyword)
    # 刷新缓存
    logger.info("违禁词缓存待刷新（关键词已更新）: %s", keyword_id)
...
def delete_keyword(
    ...
    db.commit()
    # 刷新缓存
    logger.info("违禁词缓存待刷新（关键词已删除）: %s", keyword_id)
    return {"success": True, "message": "删除成功"}


@router.post("/refresh-cache", summary="刷新违禁词缓存",
             dependencies=[Depends(require_admin)])
def refresh_cache(
        db: Session = Depends(get_db),
        token: str = Depends(oauth2_scheme)):
    """刷新违禁词缓存"""
    logger.info("违禁词缓存刷新请求已记录")
    return {"success": True, "message": "缓存已刷新"}


@router.post("/init-defaults", summary="初始化默认违禁词",
             dependencies=[Depends(require_admin)])
