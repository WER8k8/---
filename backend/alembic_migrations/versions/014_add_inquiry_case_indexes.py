...
def upgrade() -> None:
    ...
    inquiry_indexes = [idx["name"]
                       for idx in inspector.get_indexes("inquiries")]
    ...
    if "idx_inquiries_status_active" not in inquiry_indexes:
        op.create_index(
            "idx_inquiries_status_active", "inquiries", [
                "status", "is_active"])

    case_indexes = [idx["name"]
                    for idx in inspector.get_indexes("case_studies")]
    ...
    if "idx_cases_status_active" not in case_indexes:
        op.create_index(
            "idx_cases_status_active", "case_studies", [
                "status", "is_active"])
