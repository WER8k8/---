...
-- ============================================
-- 20. 第三方登录绑定表 (QQ/微信/邮箱)
-- ============================================
CREATE TABLE `social_logins` (
  `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `user_id` INT UNSIGNED NOT NULL COMMENT '用户ID',
  `provider` ENUM('qq', 'wechat', 'email') NOT NULL COMMENT '登录方式：qq/wechat/email',
  `provider_user_id` VARCHAR(100) NOT NULL COMMENT '第三方用户唯一标识',
...
-- 21. 微信推送配置表
-- ============================================
CREATE TABLE `wechat_push_configs` (
  `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `app_id` VARCHAR(100) DEFAULT '' COMMENT '微信AppID',
  `app_secret` VARCHAR(100) DEFAULT '' COMMENT '微信AppSecret',
  `access_token` TEXT COMMENT '微信访问令牌',
  `token_expires_at` DATETIME NULL COMMENT '令牌过期时间',
  `target_user_ids` TEXT COMMENT '推送目标用户ID列表，逗号分隔',
  `target_openids` TEXT COMMENT '推送目标OpenID列表，逗号分隔',
  `push_enabled` TINYINT DEFAULT 1 COMMENT '是否启用推送：1是 0否',
  `push_time` VARCHAR(20) DEFAULT '09:00' COMMENT '每日推送时间',
  `push_types` VARCHAR(100) DEFAULT 'daily,alert' COMMENT '推送类型：daily日常/alert预警/all全部',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='微信推送配置表';
...
CREATE TABLE `wechat_push_logs` (
  `id` BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `push_type` VARCHAR(20) NOT NULL COMMENT '推送类型：daily/alert',
  `title` VARCHAR(200) DEFAULT '' COMMENT '推送标题',
  `content` TEXT COMMENT '推送内容',
  `target_openid` VARCHAR(100) DEFAULT '' COMMENT '目标OpenID',
  `target_user_id` INT UNSIGNED DEFAULT NULL COMMENT '目标用户ID',
...
CREATE TABLE `email_verifications` (
  ...
  `verification_type` ENUM('login', 'bind', 'reset') NOT NULL COMMENT '验证类型：login登录/bind绑定/reset重置',
  `ip_address` VARCHAR(45) DEFAULT '' COMMENT 'IP地址',
  `expires_at` DATETIME NOT NULL COMMENT '过期时间',
  `is_used` TINYINT DEFAULT 0 COMMENT '是否已使用：1是 0否',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
...
-- 插入默认微信推送配置
INSERT INTO `wechat_push_configs` (`push_enabled`, `push_time`, `push_types`) VALUES
(0, '09:00', 'daily,alert');
