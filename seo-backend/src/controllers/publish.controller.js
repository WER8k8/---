...
const createPublishTask = async (req, res) => {
  try {
    ...
    if (!Array.isArray(platform_account_ids) || platform_account_ids.length === 0) {
      return res.status(400).json({ code: 400, message: '请提供平台账号ID列表' });
    }
    ...
    res.json({
      code: 200,
      message: `成功创建${tasks.length}个发布任务`,
