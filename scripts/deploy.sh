...
COMPOSE_MIRROR="docker-compose.cncfstack.yml"
PROJECT_NAME="youding-seo"
BACKUP_DIR="./backups"
HEALTH_CHECK_RETRIES=30
HEALTH_CHECK_INTERVAL=10

# 藏云阁镜像加速（国内）：https://cncfstack.com/p/assets/docs/cncfstack/image/
compose() {
    docker compose -f "$COMPOSE_FILE" -f "$COMPOSE_MIRROR" "$@"
}
...
backup_current() {
    ...
    docker compose -f "$COMPOSE_FILE" -f "$COMPOSE_MIRROR" ps -q 2>/dev/null | while read container_id; do
...
run_migrations() {
    log_step "执行数据库迁移..."
    compose run --rm backend alembic upgrade head 2>/dev/null || log_warn "数据库迁移跳过"
    log_info "数据库迁移完成"
}
