...
commits: f76947e4f6d985680cec2bf860c9fe0ac836ae7e..8cd5f2a0b53532d3cc7ade39ae62706e94ef9b4f
---
...
**Verification** — pytest annex 相关 **21 passed**；`verify_annex_domain_merge` / `verify_annex_work_mode` / `verify_annex_identity_strip` **PASS**；`orchestration_selfcheck` **19/19**；**live HTTP**：GP-A `plan=9773923d-a568-4dd7-81ad-16eb31592f94` `source=L1_template` `nodes=9` `dispatched=True`；GP-B `plan=daf12e6f-0c6f-404c-9fce-866eeb2a07fa` `source=L1_template` `nodes=3`。

**Journey log** — ① Compose 工作区：智能判断→当前 AGENTS 路径 + `feat/annex-domain-merge`（非嵌套 worktree）。② 范围：智能判断→P0 收口 + GP-A 闭环 + P1 核心。③ 评审 critical：提交须 path-filter；live HTTP 依赖后端进程。④ 剥身份在 `_external`，主仓门禁为静态标记 + 可移植相对路径。⑤ 旧「附属执行台」叙事已从业务菜单与能力描述移除。
...
- [x] T6: 门禁与回归 — acceptance: pytest annex、verify_annex_*、selfcheck 19/19、live GP-A/B（covers: S2）
