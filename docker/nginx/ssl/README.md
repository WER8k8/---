...
## Staging 自签（ARCH-04 · 本地彩排）

- 域名：`demo.youding.local`（`/etc/hosts` 或 DNS 指向 staging）
- 生成：`python scripts/gen-staging-ssl.py` → `fullchain.pem` / `privkey.pem`
- 校验：`python scripts/validate-arch-04-staging-https.py`
- 生产：Owner 提供真实 HTTPS 域后由 Certbot 替换本目录证书
